...
% Output:
%      A_hat - the low rank matirx with the size of m x n (num_pixels x bands)
%      E_hat - the sparse matirx with the size of m x n (num_pixels x bands)  
...
% initialize
Y = D;
norm_two = lansvd(Y, 1, 'L');  % computes the K largest singular values(where K=1)
...
converged = false;
stopCriterion = 1;
sv = 10;  % sv is the dimension of the  predicted principal singular space whose singular values are larger than a given threshold.
while ~converged       
    iter = iter + 1;
    % update E
    ...
    diagS = diag(S);
    svp = length(find(diagS > 1/mu)); % svp is the number of singular values in the sv singular values that are larger than 1/mu
    if svp < sv
        sv = min(svp + 1, n);
    else
